...
function TableMenu({row, read, unread}) {
  ...
  const handleClick = (event) => {
    ...
    return (
      <React.Fragment>
        <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center', width: '1em' }}>
        ...
        >
          <MenuItem>
            <Button onClick={() => deletePetitionfn(row.id)} size='small' startIcon={<DeleteForever />} >
              Delete
            </Button>
          </MenuItem>
          <MenuItem>
            {
              row.read === true ? (
                <Button onClick={() => unread(row.id)} size='small' startIcon={<MarkAsUnread />} >
                  Mark as unread
                </Button>
              ): (
                  <Button onClick={() => read(row.id)} size='small' startIcon={<MarkEmailRead />} >
                    Mark as read
                  </Button>
              )
            }
          ...
          <MenuItem>
            {
              row.isImportant === true ? (
                <Button onClick={() => toggleImportancefn(row.id)} size='small' startIcon={<LowPriority />} >
                  Mark as not important
                </Button>
              ): (
                  <Button onClick={() => toggleImportancefn(row.id)} size='small' startIcon={<PriorityHigh />} >
                    Mark as important
                  </Button>
              )
            }
      ...
      <Table size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell size='small' align="center">Solicitation date</TableCell>
            <TableCell size='small' align="center">User</TableCell>
            <TableCell size='small' align="center">Animal</TableCell>
            <TableCell size='small' align="center">Location</TableCell>
            <TableCell size='small'align="center"></TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {petitions.map((p) => (
            <TableRow key={p.id}>
              <TableCell  size='small' align="center">
                {
                  (p.isImportant === false)  ? (
                    ...
                    <div style={{display: 'flex', alignItems:'center', justifyContent:'center', width:'.1em'}}><PriorityHigh sx={{color:'red'}}/></div>
                  )
                }
              </TableCell>
              <TableCell size='small'align="center">{getDate(p.createdAt)}</TableCell>
              <TableCell  size='small' align="center">{p.user.name} {p.user.lastName}</TableCell>
              <TableCell size='small' align="center"><Link to={`/home/animals/${p.animal.id}`}>{p.animal.name}</Link></TableCell>
              <TableCell   size='small' align="center">{p.user.location}</TableCell>
              <TableCell  size='small' align="center">
                {
                  p.read === true ? (
              ...
              <TableCell size='small' align='center'>
                <TableMenu row={p} read={readAdoptionfn} unread={unreadAdoptionfn}/>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
