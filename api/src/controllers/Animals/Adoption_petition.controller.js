...
async function addPetition(req, res) {
  ...
  try {
    ...
    let exists = await Adoption_petition.findOne({
      where: { animalId: animId, userId: userId },
    });
    //console.log(exists);
    if (exists)
      return res
        .status(400)
        .send({ err: "You already requested this animal's adoption" });
    let petition = await animal.createAdoption_petition({
      topic: topic,
...
async function getAllPetitions(req, res) {
  try {
    ...
    if (!petitions || petitions.length < 1)
...
async function toggleImportantPetition(req, res) {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ success: "Petition important status changed" });
  } catch (error) {
    console.log(error);
    return res.status(500).send(error);
  }
